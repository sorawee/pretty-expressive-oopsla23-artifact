...
theorem Resolve_optimal_bigtext
      (h_print : Resolve F (Doc.bigtext l) c i ml)
      (h_widen : Widen (Doc.bigtext l) D) (h_mem : d_choiceless ∈ D) 
      (h_render : MeasRender F d_choiceless c i m)
      (h_y : m.y ≤ F.W) (h_x : m.x ≤ F.W) :
      ∃ ms h, ml = MeasureSet.set ms h ∧ ∃ m_better, m_better ∈ ms ∧ dominates F m_better m := by
  cases h_widen 
  ...
  case bigtext_set h' => 
    ...
    exists [m], (by simp)
    simp [dominates_refl]
  case bigtext_taint h_bad h_render' => 
    cases h_render
    simp at h_y h_x 
    cases h_bad 
    case inl h_bad => linarith
